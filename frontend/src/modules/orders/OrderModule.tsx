...
// ── Type ──────────────────────────────────────────────────────────
interface Order {
  id: string; customer: string; origin: string; dest: string;
  items: number; value: number; status: string; sla: string; priority: string;
}
...
const PRIO_COLOR: Record<string, string> = { express:'#ef4444', standard:'#00d4ff', economy:'#10b981' };

const PIPELINE = [
  { stage:'NEW',        color:'#7c3aed' },
  { stage:'CONFIRMED',  color:'#00d4ff' },
  { stage:'IN TRANSIT', color:'#2563eb' },
  { stage:'DELIVERED',  color:'#10b981' },
];
...
export default function OrderModule() {
  ...
  const [form, setForm] = useState({
    customer:'', origin:'', dest:'', items:'', value:'', priority:'standard', sla:'',
  });
  ...
  // ── Handlers ────────────────────────────────────────────────────
  const handleCreate = () => {
    if (!form.customer || !form.origin || !form.dest) {
    ...
    const newOrder: Order = {
      ...
      status:   'Pending',
      sla:      form.sla || new Date(Date.now() + 3 * 86400000).toISOString().slice(0, 10),
      priority: form.priority,
    };
    setOrders(prev => [newOrder, ...prev]);
    setForm({ customer:'', origin:'', dest:'', items:'', value:'', priority:'standard', sla:'' });
  ...
  const handleUpdateStatus = () => {
    ...
    toast.success(`${selected.id} → ${editStatus}`);
    setEditNote('');
  };
  ...
  return (
    <div style={{ height:'100%', overflowY:'auto' }}>
      <PageIntro
        ...
        color="#06b6d4"
        description="Full order lifecycle from creation to delivery. Create orders, confirm, dispatch, track in real-time and auto-generate invoices."
        features={['Order creation & validation','Pipeline stage management','SLA date tracking','Priority-based routing','Status history timeline','Invoice auto-generation']}
      />
      ...
      <div style={{ padding:'20px 20px 0', marginBottom:16 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:20 }}>
          {PIPELINE.map((p, i) => {
            const count = counts[p.stage as keyof typeof counts] ?? 0;
            return (
              <div key={p.stage} style={{ position:'relative' }}>
                <div style={{ padding:'14px 16px', borderRadius:12, background:`${p.color}08`, border:`1px solid ${p.color}25` }}>
                  ...
                  <div style={{ fontSize:28, fontWeight:800, color:p.color, fontFamily:'var(--font-mono)' }}>{count}</div>
        ...
        <div style={{ display:'flex', gap:10, alignItems:'center' }}>
          ...
          <ActionModal title="Create New Order" color="#06b6d4" trigger={
            ...
            <FormInput label="Customer Name"      placeholder="e.g. Tata Motors Ltd" value={form.customer} onChange={v => setForm(p => ({ ...p, customer:v }))} />
            ...
            <FormInput label="Total Items"        type="number" placeholder="e.g. 42"     value={form.items}    onChange={v => setForm(p => ({ ...p, items:v }))}    />
            <FormInput label="Order Value (₹)"    type="number" placeholder="e.g. 824000" value={form.value}    onChange={v => setForm(p => ({ ...p, value:v }))}    />
            <FormSelect label="Priority" options={['economy','standard','express']} value={form.priority} onChange={v => setForm(p => ({ ...p, priority:v }))} />
            <FormInput label="SLA Date" type="date" value={form.sla} onChange={v => setForm(p => ({ ...p, sla:v }))} />
            <SubmitBtn label="Create Order" color="#06b6d4" onClick={handleCreate} />
          </ActionModal>
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:12, padding:'12px 20px 20px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          ...
          {filtered.map(o => {
            const cfg  = STATUS_CFG[o.status] ?? { color:'#888', bg:'rgba(136,136,136,0.08)', icon:Clock };
            const Icon = cfg.icon;
            const isSel = selected?.id === o.id;
            return (
              <div key={o.id} onClick={() => { setSelected(o); setEditStatus(o.status); }}
                style={{
                ...
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      ...
                      <span style={{ padding:'2px 8px', borderRadius:99, fontSize:9, fontWeight:700, background:`${PRIO_COLOR[o.priority] ?? '#888'}18`, color:PRIO_COLOR[o.priority] ?? '#888' }}>
                  ...
                  <div style={{ textAlign:'right', flexShrink:0 }}>
                    <div style={{ fontSize:14, fontWeight:800, color:'#10b981', fontFamily:'var(--font-mono)' }}>
                      {o.value >= 100000 ? `₹${(o.value / 100000).toFixed(1)}L` : `₹${o.value.toLocaleString('en-IN')}`}
                    </div>
              ...
              <div style={{ padding:'16px' }}>
                <div style={{ display:'grid', gap:10, marginBottom:16 }}>
                  {([
                    ...
                    ['Priority',  selected.priority,                                PRIO_COLOR[selected.priority] ?? '#888'],
                    ['Status',    selected.status,                                  STATUS_CFG[selected.status]?.color ?? '#888'],
                    ...
                    <div key={k} style={{ display:'flex', justifyContent:'space-between', padding:'8px 10px', borderRadius:8, background:'rgba(255,255,255,0.03)' }}>
                      ...
                      <span style={{ fontSize:11, fontWeight:700, color:c, textAlign:'right', maxWidth:160, overflow:'hidden', textOverflow:'ellipsis' }}>{v}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display:'grid', gap:8 }}>
                  <ActionModal title={`Update ${selected.id}`} color="#06b6d4" trigger={
                    ...
                    <FormSelect label="New Status" options={['Pending','Confirmed','In Transit','Delivered','Cancelled']} value={editStatus} onChange={v => setEditStatus(v)} />
                    <FormInput  label="Note" placeholder="e.g. Customer confirmed delivery" value={editNote} onChange={v => setEditNote(v)} />
                    <SubmitBtn label="Save Changes" color="#06b6d4" onClick={handleUpdateStatus} />
                  </ActionModal>
